...
class EventEmiter extends UidGenerator {
    ...
    when(events, listener, changeRequired) {
        ...
        events.trim().split(' ').forEach(event => {
            ...
            eventData = eventData ? eventData : this.eventsManager[event] = {
                wrapperCollection: {},
                lastValue: undefined,
            };
            const wrapper = {
    ...
    #emitEvent(event, value, originalEvent) {
        let eventData = this.eventsManager[event];
        if (eventData == undefined)
            return; // this event is not yet listened
        if (value === undefined) {
    ...
    remove(e) {
        ...
        events.split(' ').forEach((event) => {
            ...
            if (eventData == undefined)
                return; // this event is not yet listened to
            const wrapperCollection = eventData.wrapperCollection;
            const wrapper = wrapperCollection[uid];
            if (wrapper == undefined)
                return;
            delete wrapperCollection[uid];
        });
    }
}
